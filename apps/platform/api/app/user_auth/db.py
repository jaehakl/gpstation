...
from sqlalchemy import Boolean, CheckConstraint, DateTime, ForeignKey, Index, LargeBinary, Numeric, Text, UniqueConstraint, func, text
from sqlalchemy import Enum as SAEnum
from sqlalchemy.dialects.postgresql import INET, JSONB, UUID
from sqlalchemy.orm import Mapped, mapped_column, relationship

from db import Base, TimestampMixin, uuid_text
...
class User(TimestampMixin, Base):
    ...
    credit_withdrawable: Mapped[Decimal] = mapped_column(Numeric(18, 6), nullable=False, server_default=text("0"))
    trust_score: Mapped[Decimal] = mapped_column(Numeric(8, 3), nullable=False, server_default=text("0"))
    trust_tier: Mapped[str] = mapped_column(Text, nullable=False, server_default=text("'standard'"))
    success_job_count: Mapped[int] = mapped_column(nullable=False, server_default=text("0"))
    failed_job_count: Mapped[int] = mapped_column(nullable=False, server_default=text("0"))
    disputed_job_count: Mapped[int] = mapped_column(nullable=False, server_default=text("0"))
