...
const SYSTEM_PROMPT = [
  'You are a Japanese language example sentence writer.',
  'Generate exactly one natural Japanese sentence that can directly follow the given base sentence.',
  'The sentence must include the required Japanese word exactly as provided.',
  'Return only the Japanese sentence. Do not include explanations, translations, markdown, bullets, or quotation marks.',
  'Keep the sentence short and suitable for a language-learning example.',
].join('\n');
...
export async function generateNextJapaneseSentence(
  ...
  return requestLlmAnswer(
    {
      ...
      prompt: [
        `Base Japanese sentence: ${sourceSentence.trim()}`,
        `Required Japanese word: ${seedWord.trim()}`,
        '',
        'Write the next Japanese sentence.',
      ].join('\n'),
      max_tokens: 80,
      temperature: 0.4,
    },
...
export async function generateSdxlPromptsFromSentences(
  sourceSentence: string,
  ...
  const translationAnswer = await requestLlmAnswer(
    {
      ...
      prompt: [
        `Previous Japanese sentence: ${sourceSentence.trim()}`,
        `Current Japanese sentence: ${generatedSentence.trim()}`,
        '',
  ...
  const previousSentence = getJsonString(
    translated,
    'previous_sentence',
    '이전 문장 번역이 비어 있습니다.',
  );
  const currentSentence = getJsonString(
    translated,
    'current_sentence',
    '현재 문장 번역이 비어 있습니다.',
  );

  const promptAnswer = await requestLlmAnswer(
    {
  ...
  const prompts = parseJsonObject(promptAnswer, 'SDXL 프롬프트 응답을 해석하지 못했습니다.');
